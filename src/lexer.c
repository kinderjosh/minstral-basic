...
Token lex_next_token(Lexer *lex) {
    ...
    switch (lex->cur) {
        ...
        case '=':
            if (peek(lex, 1) == '=')
                return create_and_step(lex, TOK_EQ, "==");

            return create_and_step(lex, TOK_EQUAL, "=");
        case '@': return create_and_step(lex, TOK_AT, "@");
        ...
        case '%': return create_and_step(lex, TOK_PERCENT, "%");
        case '<':
            if (peek(lex, 1) == '<')
                return create_and_step(lex, TOK_SHL, "<<");
            else if (peek(lex, 1) == '=')
        ...
        case '>':
            if (peek(lex, 1) == '>')
                return create_and_step(lex, TOK_SHR, ">>");
            else if (peek(lex, 1) == '=')
                return create_and_step(lex, TOK_GTE, ">=");

            return create_and_step(lex, TOK_GT, ">");
        case '&': return create_and_step(lex, TOK_AND, "&");
        case '|': return create_and_step(lex, TOK_OR, "|");
        case '^': return create_and_step(lex, TOK_XOR, "^");
        case '~': return create_and_step(lex, TOK_NOT, "~");
        case '!':
            if (peek(lex, 1) == '=')
